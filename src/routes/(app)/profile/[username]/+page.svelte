<script lang="ts">
	...
	import { MessageCircle, _Calendar, Package, Star, LogOut } from 'lucide-svelte'
	...
	const _username = $derived($page?.params.username)
			...
			<div class="max-w-4xl mx-auto">
				<nav class="flex" aria-label="Profile tabs">
					...
					>
						...
						<span class="text-xs font-normal opacity-70">{listings?.length ?? 0}</span>
					</button>
					<button
						class="flex-1 py-3 px-2 border-b-2 font-medium text-sm transition-all relative
							{activeTab === 'reviews' 
					...
					>
						...
						<span class="text-xs font-normal opacity-70">{reviews?.length ?? 0}</span>
					</button>
					<button
						class="flex-1 py-3 px-2 border-b-2 font-medium text-sm transition-all relative
							{activeTab === 'about' 
					...
					{:else if activeTab === 'about'}
						...
						<div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
							...
							<div class="space-y-3 mt-6">
								...
								{#if profile?.verification_badges??.length ?? 0 > 0}
									<div class="pt-3">
