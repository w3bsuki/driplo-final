<script lang="ts">
	...
	// Type guards to safely access data that might be SelectQueryError
	function isValidData<T>(data: any): data is T {
		return data && typeof data === 'object' && !('error' in data) && !('code' in data) && !('details' in data) && !('hint' in data) && !('message' in data);
	}

	const brandProfile = $derived(isValidData(data.brandProfile) ? data.brandProfile as any : null);
	const user = $derived(brandProfile && 'user' in brandProfile && isValidData(brandProfile.user) ? brandProfile.user as any : null);
	const listings = $derived(isValidData(data.listings) ? (data.listings as any[]) : []);
	const stats = $derived(isValidData(data.stats) ? data.stats as any : null);
	const reviews = $derived(isValidData(data.reviews) ? (data.reviews as any[]) : []);

	const socialLinks = $derived(brandProfile ? [
...
<svelte:head>
	<title>{brandProfile?.brand_name || 'Brand'} | Driplo Brand</title>
	<meta name="description" content="{brandProfile?.brand_description || `Shop from ${brandProfile?.brand_name || 'this brand'} on Driplo`}" />
</svelte:head>
	...
	{#if brandProfile?.brand_cover_url}
		<div class="relative h-48 md:h-64 lg:h-80">
			<img 
				...
				alt="{brandProfile.brand_name || 'Brand'} cover"
		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div class="flex flex-col sm:flex-row items-start sm:items-center gap-6">
					...
					<div class="flex-1">
						<div class="flex items-center gap-3 mb-2">
							<h1 class="text-3xl font-bold text-gray-900">{brandProfile?.brand_name || 'Unknown Brand'}</h1>
						...
						<div class="flex flex-wrap gap-4 text-sm text-gray-500">
							...
							{#if user?.seller_rating}
								<div class="flex items-center gap-1">
									...
									{user.seller_rating.toFixed(1)} ({user.seller_rating_count || 0} reviews)
								</div>
							{/if}
						</div>
					</div>
						...
						{#each socialLinks as link}
							...
							>
								{#if link.icon}
									...
									<IconComponent class="w-5 h-5 text-gray-700" />
			...
			<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 py-6 border-t">
				<div class="text-center">
					<p class="text-3xl font-bold text-gray-900">{stats?.total_listings || 0}</p>
			...
			<div class="border-t">
				<nav class="flex gap-8">
					...
					>
						Products ({stats?.total_listings || 0})
					</button>
					<button
	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		{#if activeTab === 'products'}
			...
			{#if listings.length > 0}
				...
				{#if stats?.total_listings && stats.total_listings > 12 && user?.username}
		...
		{:else if activeTab === 'about'}
			...
			<div class="max-w-3xl">
				<div class="bg-white rounded-xl p-6 shadow-sm">
					<h2 class="text-xl font-semibold mb-4">About {brandProfile?.brand_name || 'this brand'}</h2>
					...
					<div class="grid sm:grid-cols-2 gap-6">
						...
						<div>
							...
							<p class="text-gray-600">
								{#if brandProfile?.created_at}
									{new Date(brandProfile.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
								{:else}
									Not available
								{/if}
							</p>
		...
		{:else if activeTab === 'reviews'}
			...
			{#if reviews.length > 0}
				<div class="space-y-4 max-w-3xl">
					{#each reviews as review}
						<div class="bg-white rounded-xl p-6 shadow-sm">
							<div class="flex items-start gap-4">
								<img 
									src={review.reviewer?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${review.reviewer?.username || 'default'}`}
								...
								<div class="flex-1">
									<div class="flex items-center justify-between mb-2">
										<div>
											<h4 class="font-medium text-gray-900">{review.reviewer?.full_name || review.reviewer?.username || 'Anonymous'}</h4>
											<div class="flex items-center gap-2 text-sm text-gray-500">
												<div class="flex">
													{#each Array(5) as _, i}
														<Star 
															class="w-4 h-4 {i < (review.rating || 0) ? 'text-yellow-500 fill-current' : 'text-gray-300'}"
														/>
													{/each}
												</div>
												<span>·</span>
												<span>{review.created_at ? getRelativeTime(review.created_at) : 'Recently'}</span>
											</div>
										</div>
									</div>
					...
					{#if stats?.total_reviews && stats.total_reviews > 5}
						<div class="text-center">
