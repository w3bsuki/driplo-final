<script lang="ts">
	...
	// Search state
	let _searchFocused = $state(false);
	...
	async function loadMoreItems() {
		...
		try {
			...
			if (data?.filters.sizes?.length ?? 0 > 0) params?.set('sizes', data?.filters.sizes?.join(','));
			if (data?.filters.brands?.length ?? 0 > 0) params?.set('brands', data?.filters.brands?.join(','));
			if (data?.filters.conditions?.length ?? 0 > 0) params?.set('conditions', data?.filters.conditions?.join(','));
