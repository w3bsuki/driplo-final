  ...
  try {
    ...
    if (format === 'csv') {
      ...
      const rows = payouts?.map(payout => [
        ...
        new Date(payout?.created_at || new Date()).toISOString(),
        payout?.processed_at ? new Date(payout?.processed_at || new Date()).toISOString() : '',
        payout?.status,
        (payout as any)?.seller?.email || '',
        (payout as any)?.seller?.username || '',
        payout?.seller_revtag || '',
        (payout as any)?.transaction?.buyer?.email || '',
        (payout as any)?.transaction?.buyer?.username || '',
        (payout as any)?.transaction?.listing?.title || '',
        (payout as any)?.transaction?.amount || 0,
        (payout as any)?.transaction?.platform_fee_amount || 0,
        payout?.amount,
        payout?.processed_by || '',
