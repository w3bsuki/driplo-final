#!/usr/bin/env python3
"""
Working TypeScript Error Log Parser
Parses all TypeScript error log files and creates a complete JSON analysis.
Correctly handles the format where file path is on one line, error/warning on the next.
"""
...
class TypeScriptErrorParser:
    def __init__(self, base_path: str):
        ...
        self.ansi_escape = re.compile(r'\x1b\[(?:\d+(?:;\d+)*)?m')
        ...
        self.category_patterns = {
            ...
            'accessibility_warnings': [
                ...
                r"Visible, non-interactive elements with a click event",
                r"should not be assigned mouse or keyboard event listeners"
            ]
        }
    ...
    def normalize_path(self, path_with_ansi: str) -> str:
        """Extract and normalize file path from ANSI-colored path."""
        # Pattern: k:\base\[32mrelative[39m:line:col
        match = re.search(r'([kK]:[^\\]+)\\?\x1b\[32m([^\x1b]+)\x1b\[39m', path_with_ansi)
        if match:
            base_path = match.group(1)
            relative_path = match.group(2)
            full_path = f"{base_path}\\{relative_path}"
            return full_path.replace('k:\\', 'K:\\').replace('/', '\\')
        
        # Fallback: just strip ANSI codes
        return self.strip_ansi_codes(path_with_ansi).replace('k:\\', 'K:\\')
    ...
    def parse_file(self, file_path: str) -> Tuple[int, int]:
        ...
        try:
            ...
            while i < len(lines):
                ...
                # Look for file path pattern: k:\path\[32mrelative[39m:line:col
                ...
                if file_match and i + 1 < len(lines):
                    ...
                    # Check the next line for error/warning
                    next_line = lines[i + 1].strip()
                    
                    # Check for error
                    if next_line.startswith('\x1b[31mError\x1b[39m:'):
                        message = next_line.replace('\x1b[31mError\x1b[39m:', '').strip()
                        # Remove trailing (ts) or similar
                        ...
                        error_key = f"{self.strip_ansi_codes(message)}"
                        ...
                        i += 2  # Skip the error line
                        continue
                    
                    # Check for warning
                    elif next_line.startswith('\x1b[33mWarn\x1b[39m:'):
                        message = next_line.replace('\x1b[33mWarn\x1b[39m:', '').strip()
                        # Remove trailing (svelte) or similar
                        message = re.sub(r'\s*\([^)]*\)$', '', message)
    ...
    def find_duplicates(self) -> List[Dict[str, Any]]:
        ...
        for message, occurrences in self.duplicated_errors.items():
...
def main():
    ...
    print("Starting comprehensive TypeScript error analysis - WORKING VERSION...")
    ...
    if analysis['summary']['totalErrors'] > 0:
        ...
        for i, error in enumerate(parser.errors[:5]):
            print(f"  {i+1}. {error['file']}:{error['line']}:{error['column']} - {error['message'][:100]}")
    
    if analysis['summary']['totalWarnings'] > 0:
        print(f"\nSample warnings found:")
        for i, warning in enumerate(parser.warnings[:5]):
            print(f"  {i+1}. {warning['file']}:{warning['line']}:{warning['column']} - {warning['message'][:100]}")
