#!/usr/bin/env python3
"""
Comprehensive TypeScript Error Log Parser - FINAL VERSION
Parses all TypeScript error log files and creates a complete JSON analysis.
Handles the actual svelte-check output format with proper ANSI escape codes.
"""
...
class TypeScriptErrorParser:
    def __init__(self, base_path: str):
        ...
        # ANSI color code regex - handles both \x1b and \[ formats
        self.ansi_escape = re.compile(r'\x1b\[(?:\d+(?:;\d+)*)?m|\[(?:\d+(?:;\d+)*)?m')
        
        # File path pattern: k:\path\[32mrelative_path[39m:line:col
        self.file_path_pattern = re.compile(
            r'([kK]:[^\\]+)\\?\x1b\[32m([^\x1b]+)\x1b\[39m:(\d+):(\d+)'
        )
        
        # Error pattern: [31mError[39m: message
        self.error_pattern = re.compile(r'\x1b\[31mError\x1b\[39m:\s*(.+?)(?:\s*\([^)]*\))?$')
        
        # Warning pattern: [33mWarn[39m: message  
        self.warning_pattern = re.compile(r'\x1b\[33mWarn\x1b\[39m:\s*(.+?)(?:\s*\([^)]*\))?$')
    ...
    def parse_file(self, file_path: str) -> Tuple[int, int]:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                # Check for file path pattern first
                ...
                # Check for error pattern in the same line (after file path)
                ...
                # Check for warning pattern in the same line (after file path)
    ...
    def find_duplicates(self) -> List[Dict[str, Any]]:
        ...
        # Group by message only (ignore line/column for duplicate detection)
        ...
        for error_key, occurrences in self.duplicated_errors.items():
            ...
            message_groups[message].extend(occurrences)
        
        for message, occurrences in message_groups.items():
...
def main():
    ...
    print("Starting comprehensive TypeScript error analysis - FINAL VERSION...")
